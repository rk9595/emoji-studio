"""Start the bounded quality pilot independently of the SSH transport."""
...
def worker(seconds):
    with (ROOT / "quality-bootstrap.log").open("ab", buffering=0) as log:
        result = subprocess.run(
            [
                ...
                f"{seconds}s",
                "bash",
                "scripts/remote_quality_pilot.sh",
    ...
    temporary = ROOT / "quality.exit.tmp"
    temporary.write_text(str(result.returncode))
    temporary.replace(ROOT / "quality.exit")
